...
app.get("/docentes/:matricula", (req, res) => {
  ...
  db.query(
    "SELECT * FROM docentes WHERE matricula = ?",
    [matricula],
    (err, results) => {
      if (err) {
...
app.post("/docentes/criar", (req, res) => {
  ...
  const sql =
    "INSERT INTO docentes (matricula_doc, nome_doc, email_doc, data_nasci_doc, data_adimissao_doc, situacao_doc, area_concurso_doc, status_doc) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  console.log(status);
...
app.put("/docentes/editar/:matricula", (req, res) => {
  ...
  const sql =
    "UPDATE docentes SET nome_doc=?, email_doc=?, data_nasci_doc=?, data_adimissao_doc=?, situacao_doc=?, area_concurso_doc=?, status_doc=? WHERE matricula_doc=?";
  db.query(
    sql,
    [
      nome,
